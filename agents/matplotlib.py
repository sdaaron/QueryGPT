...
from agents.plot_prompt import (
    PREFIX,
    SUFFIX_NO_DF,
    SUFFIX_WITH_DF,
    FORMAT_INSTRUCTIONS
...
def create_matplotlib_dataframe_agent(
    llm: BaseLLM,
    df: Any,
    ...
    **kwargs: Dict[str, Any],
    ...
    tools = [PythonAstREPLTool(locals={"df": df, "plt": plt, "pd": pd})]
    now = pd.Timestamp.now().strftime("%Y-%m-%d_%H-%M-%S")
    prompt = ZeroShotAgent.create_prompt(
        tools, prefix=prefix.format(now=f"images/{now}"), suffix=suffix_to_use, format_instructions=FORMAT_INSTRUCTIONS,

...
async function onConversation() {
  ...
  function convertObjectToArray(obj: any) {
    ...
    const result = values[0].map((_, index: number) => {
      const newObj: any = {}
      keys.forEach((key) => {
        newObj[key] = values[keys.indexOf(key)][index]
      })
      return newObj
    })
  ...
  try {
    ...
    const fetchChatAPIOnce = async () => {
      ...
      const result = active.value ? `${res.data.response}\n\n数据来源：\n\n>${res.data.source_documents.join('>')}` : res.data.response.text.split('|    |')[0]
      const { text, ...data } = res.data.response.data
      const type = res.data.response.chart_type

      const tableData = convertObjectToArray(data)
      const { 日期, ...newData } = data

      const xAxisData = 日期
      const seriesData = Object.keys(newData).map(key => ({
        name: key,
        //  type: type.toLowerCase() === 'none' ? 'line' : type.toLowerCase(),
        type: 'bar',
				 data: newData[key],
      }))
...
async function onRegenerate(index: number) {
  ...
  try {
    ...
    const fetchChatAPIOnce = async () => {
      ...
      const result = active.value ? res.data.response.text : res.data.response.text
      updateChat(
        +uuid,
        dataSources.value.length - 1,
        {
...
const footerClass = computed(() => {
  let classes = ['p-4', 'bg-[#EEEEEE]']
  if (isMobile.value)
    classes = ['sticky', 'left-0', 'bottom-0', 'right-0', 'p-2', 'pr-3', 'overflow-hidden', 'bg-[#EEEEEE]']
  return classes
})
...
<template>
  <div class="flex flex-col w-full h-full ">
    ...
    />
    <main class="flex-1 overflow-hidden bg-[#EEEEEE]">
